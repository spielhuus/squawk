...
const std::array< upnp::NodeType::Enum, 4 > Didl::CLASS_CONTAINER = {
    upnp::NodeType::Enum::album, upnp::NodeType::Enum::artist, upnp::NodeType::Enum::folder,
    upnp::NodeType::Enum::serie
};

Didl::Didl ( data::redis_ptr redis ) : redis_ ( redis ), result_ ( 0 ) {
...
void Didl::write ( const std::string& key, const std::map< std::string, std::string >& values ) {
    ...
    const std::string _cds_uri = "http://192.168.0.1:9010/"; //TODO from database

    if ( values.find ( param::CLASS ) == values.end() )
    { return; }

    upnp::NodeType::Enum _type = upnp::NodeType::parse ( values.at ( param::CLASS ) );
    ...
    //TODO attr ( &doc_, _container_n, DIDL_ATTRIBUTE_CHILD_COUNT, std::to_string ( data::children_count ( redis_, key ) ) );

    if ( _type == upnp::NodeType::Enum::episode ) {
    ...
    } else if ( _type == upnp::NodeType::Enum::audio ) {
        auto _res_n = element<rapidxml_ns::xml_node<>> ( &doc_, _container_n, "res", fmt::format ( "{0}/res/{1}{2}",
                      _cds_uri, key, values.at ( "ext" ) ) );
    ...
    } else if ( _type == upnp::NodeType::Enum::image || _type == upnp::NodeType::Enum::cover ) {
        auto _res_n = element<rapidxml_ns::xml_node<>> ( &doc_, _container_n, "res", fmt::format ( "{0}/res/{1}{2}",
                      _cds_uri, key, values.at ( "ext" ) ) );
    ...
    } else if ( _type == upnp::NodeType::movie ) {
        auto _res_n = element<rapidxml_ns::xml_node<>> ( &doc_, _container_n, "res", fmt::format ( "{0}/res/{1}{2}",
                      _cds_uri, key, values.at ( "ext" ) ) );
