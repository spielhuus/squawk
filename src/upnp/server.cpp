...
Server::Server ( const std::string& redis, /** @param redis redis host */
                 const std::string& port /** @param port the redis port. */ )

    : redis_data_ ( data::make_connection ( redis, std::stoi ( port ), REDIS_DATABASE::FS ) ),
      redis_config_ ( data::make_connection ( redis, std::stoi ( port ), REDIS_DATABASE::CONFIG ) ),
      listen_address_ ( redis_config_->get ( config::UPNP_LISTEN_ADDRESS ) ),
      listen_port_ ( redis_config_->get ( config::UPNP_LISTEN_PORT ) ),
      multicast_address_ ( redis_config_->get ( config::UPNP_MULTICAST_ADDRESS ) ),
      multicast_port_ ( redis_config_->get ( config::UPNP_MULTICAST_PORT ) ),
      name_ ( redis_config_->get ( config::NAME ) ),
      uuid_ ( redis_config_->get ( config::UUID ) ),
      http_server_ ( std::shared_ptr< http::Server< http::HttpServer > > (
                         new http::Server< http::HttpServer > ( redis_config_->get ( config::UPNP_LISTEN_ADDRESS ), redis_config_->get ( config::UPNP_LISTEN_PORT ) ) ) ) {

    if ( !sub_.connect ( redis, std::stoi ( port ) ) ) {
        ...
        sub_.subscribe ( "CDS_SUBSCRIBE", [this] ( const std::string & topic, const std::string & msg ) {
            ...
            //persist the url
            //TODO redis_->command ( {redis::ZADD, data::make_key ( "upnp", "event", "cds" ), "0", msg } );
            //fire initial event
            ...
            _request.parameter ( "SID", fmt::format ( "uuid:{}", uuid_ ) );
    ...
    //create web server
    spdlog::get ( LOGGER )->debug ( "Start web server:{}:{}", listen_address_, listen_port_ );

    http_server_->bind ( http::mod::Match<> ( "^\\/root.xml$" ),
                         http::mod::Exec ( std::bind ( &upnp::Server::description, this, _1, _2 ) ),
                         http::mod::Http() );

    http_server_->bind ( http::mod::Match<> ( "^\\/cds.xml$" ),
    http::mod::Exec ( [] ( http::Request&, http::Response& response ) -> http::http_status {
        response << CDS_DESCRIPTION;
        response.parameter ( http::header::CONTENT_TYPE, http::mime::mime_type ( http::mime::XML ) );
        return http::http_status::OK;
    } ),
    http::mod::Http() );

    http_server_->bind ( http::mod::Match<> ( "^\\/cms.xml" ),
    http::mod::Exec ( [] ( http::Request&, http::Response& response ) -> http::http_status {
        response << CMS_DESCRIPTION;
        response.parameter ( http::header::CONTENT_TYPE, http::mime::mime_type ( http::mime::XML ) );
        return http::http_status::OK;
    } ),
    http::mod::Http() );

    http_server_->bind ( http::mod::Match<> ( "^/ctl/cds$" ),
                         http::mod::Exec ( std::bind ( &upnp::Server::cds, this, _1, _2 ) ),
                         upnp::UpnpHeaders ( &uuid_ ) );

    http_server_->bind ( http::mod::Match<> ( "^/ctl/cms$" ),
                         http::mod::Exec ( std::bind ( &upnp::Server::cms, this, _1, _2 ) ),
                         http::mod::Http() );

    http_server_->bind ( http::mod::Match< std::string > ( "^\\/+squawk([[:digit:]]+).png$", key::KEY ),
...
http::http_status Server::cds ( http::Request& request, http::Response& response ) {
    ...
    if ( request.method() == "SUBSCRIBE" ) {
        ...
        if ( !_callback.empty() ) {
            ...
            redis_data_->publish ( "CDS_SUBSCRIBE", _callback );
        ...
        response.parameter ( http::header::CONTENT_LENGTH, "0" );
        response.parameter ( "SID", fmt::format ( "uuid:{}", uuid_ ) );
        ...
        if ( !_callback.empty() ) {
            ...
            redis_data_->publish ( "CDS_UNSUBSCRIBE", _callback );
        ...
        if ( _upnp_command.type == UpnpCommand::BROWSE ) {
            ...
            if ( _upnp_command.contains ( BROWSE_FLAG ) &&
                ...
                auto _node = data::node ( redis_data_, _object_id );
                ...
                upnp::NodeType::Enum _type = ( _object_id=="root" ?
                                               upnp::NodeType::folder :
                                               upnp::NodeType::parse ( data::get ( redis_data_, _object_id, param::CLASS ) ) );

                if ( _object_id=="root" ) {

                    _didl.write ( "fs::albums", {{"key", "fs::album"},{param::PARENT, "fs:root"},{param::CLASS, param::ALBUM},{"name", "Albums"}} );
                    _didl.write ( "fs::artists", {{"key", "fs::artists"},{param::PARENT, "fs:root"},{param::CLASS, param::ARTIST},{"name", "Artists"}} );
                    _didl.write ( "fs::ebook", {{"key", "fs::ebooks"},{param::PARENT, "fs:root"},{param::CLASS, param::FOLDER},{"name", "Ebooks"}} );
                    _didl.write ( "fs::movies", {{"key", "fs::movies"},{param::PARENT, "fs:root"},{param::CLASS, param::FOLDER},{"name", "Movies"}} );

                } else if ( _object_id=="fs::album" ) {
                    ...
                    if ( _type == upnp::NodeType::album ) {
//TODO                    data::files ( redis_, _object_id, upnp::NodeType::audio, _index, _count, [this,&_didl,&_upnp_command] ( const std::string& key ) {
//                        _didl.write ( key, data::node ( redis_, key ) );
//                    } );

                    } else {
//TODO                        data::children ( redis_data_, _object_id, _index, _count, "default" /** TODO */, "asc", "", [this,&_didl,&_upnp_command] ( const std::string& key ) {
//                            _didl.write ( key, data::node ( redis_data_, key ) );
//                    } );
                    }
                }

                SPDLOG_DEBUG ( spdlog::get ( LOGGER ), _didl.str() );
                //TODO response << soap_envelope ( _didl.str(), _didl.count(), data::children_count ( redis_data_, _object_id ) );
...
http::http_status Server::cms ( http::Request& request, http::Response& response ) {
    ...
    if ( request.method() == "SUBSCRIBE" ) {
        ...
        if ( !_callback.empty() ) {
            ...
            redis_data_->publish ( "CMS_SUBSCRIBE", _callback );
        ...
        response.parameter ( http::header::CONTENT_LENGTH, "0" );
        response.parameter ( "SID", fmt::format ( "uuid:{}", uuid_ ) );
        redis_data_->publish ( "CMS_UNSUBSCRIBE", request.str() );
...
//TODO    attr( &doc, root_n, "xmlns:sec", "http://www.sec.co.kr/dlna" );
    ...
    element<rapidxml_ns::xml_node<>> ( &doc, device_n, "friendlyName", name_ );
...
//    element<rapidxml_ns::xml_node<>>( &doc, device_n, "sec:ProductCap", "smi,DCM10,getMediaInfo.sec,getCaptionInfo.sec" );
    ...
    element<rapidxml_ns::xml_node<>> ( &doc, device_n, "UDN", fmt::format ( "uuid:{}", uuid_ ) );
